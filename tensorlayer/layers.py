...
class UpSampling2dLayer(Layer):
    ...
    def __init__(
        ...
        Layer.__init__(self, name=name)
        self.inputs = layer.outputs
        print("  [TL] UpSampling2dLayer %s:  method:%d align_corners:%s" %  (name, method, align_corners))
        with tf.variable_scope(name) as vs:
            self.outputs = tf.image.resize_images(self.inputs, size=[tf.shape(self.inputs)[1]*scale[0],tf.shape(self.inputs)[2]*scale[1]], method=method, align_corners=align_corners)            
        self.all_layers = list(layer.all_layers)
        self.all_params = list(layer.all_params)
        self.all_drop = dict(layer.all_drop)
        self.all_layers.extend( [self.outputs] )
